...
def calcSRMatrixApprox(
    ...
    *,
    ...
    if stable_integrals:
        # Analytic small-argument limits avoid cancellation when a sample's
        # stationary point coincides with a pixel centre. The default retains
        # the historical arithmetic for reference-scanner parity.
        u = LinCoeff_x_delta_Mat
        small = u.abs() < 0.01
        u2 = u.square()
        second = 2 * deltaMat**3 * (1 / 3 - u2 / 10 + u2**2 / 168 - u2**3 / 6480)
        first = 2j * deltaMat**2 * u * (1 / 3 - u2 / 30 + u2**2 / 840 - u2**3 / 45360)
        HighOrder2 = torch.where(small, second, HighOrder2)
        DerivativeOrder1 = torch.where(small, first, DerivativeOrder1)
...
def calcInvA(
    ...
    *,
    ...
    AFinal = calcSRMatrixApprox(
        ...
        stable_integrals=stable_integrals,
    )[0]

    GaussWeightVar = (GaussRelativeWidth * torch.pi * NumPixelsFinal**2 / MaxPhase) ** 2
